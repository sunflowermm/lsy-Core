...
function emitTools(res, id, model, tools) {
  ...
  writeSSE(res, { ...sseChunk(id, model, {}), lsy_tools: tools });
}

function emitStreamError(res, id, model, message) {
  if (!res || res.writableEnded) return;
  writeSSE(res, { ...sseChunk(id, model, {}), lsy_error: { message } });
}
  ...
  try {
    const { content, truncated } = await runLsyAgent({
      ...
      onTextDelta: (delta) => {
        ...
        writeSSE(res, sseChunk(id, modelName, first ? { role: 'assistant', content: delta } : { content: delta }));
        first = false;
      }
    });
    ...
    if (truncated) {
      total = appendTruncationNote(live, total, (note) => {
        writeSSE(res, sseChunk(id, modelName, { content: note }));
      });
    }

    if (!clientGone) {
      writeSSE(res, sseChunk(id, modelName, {}, 'stop'));
      writeSSE(res, '[DONE]');
      res.end();
    }
  } catch (err) {
    ...
    if (!clientGone && !res.writableEnded) {
      ...
      writeSSE(res, sseChunk(id, modelName, { content: `\n\n[错误] ${msg}` }));
      writeSSE(res, sseChunk(id, modelName, {}, 'stop'));
      writeSSE(res, '[DONE]');
      res.end();
    }
