import { buildWebFetchRuntime, runWebFetch } from '../../system-Core/lib/crawl/index.js';
...
export const LSY_TOOL_SPECS = [
  ...
  {
    name: 'web_search',
    description: '网页搜索（国内优先 Bing/百度，可配置 search.provider）',
    input_schema: {
      type: 'object',
  ...
  switch (name) {
    ...
    case 'web_search':
      return withTool(ctx, name, async () => {
        ...
        const max = Math.min(10, Math.max(1, Math.floor(args.maxResults || 6)));
        return runWebSearch(query, max);
      });
    case 'web_fetch':
      return withTool(ctx, name, async () => {
