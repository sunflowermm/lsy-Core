...
export const LSY_TOOL_SPECS = [
  ...
  {
    name: 'web_search',
    description: '开放域网页搜索（同 AGT web.web_search；配置 ai-workflow.crawl.webSearch）',
    input_schema: {
      type: 'object',
  ...
  switch (name) {
    case 'read':
      return withTool(ctx, name, async () => {
        ...
        const truncated = content.length > runtimeConfig.maxReadChars;
        if (truncated) content = content.slice(0, runtimeConfig.maxReadChars);
    ...
    case 'grep':
      return withTool(ctx, name, async () => {
        ...
        return tools.grep(args.pattern, args.filePath, {
          caseSensitive: false,
          lineNumbers: true,
          maxResults: runtimeConfig.grepMaxResults
        });
      });
    case 'list_files':
      return withTool(ctx, name, async () => {
