...
async function buildServiceSnapshot() {
  ...
  const provider = String(runtimeConfig.llm?.provider || '').trim();
  const profile = provider ? LLMFactory.getProviderConfig(provider) : null;
  return {
    enabled: runtimeConfig.enabled !== false,
    llm: maskLlmConfigPublic(runtimeConfig.llm || {}, profile),
    defaultQuota: runtimeConfig.defaultQuota ?? 100,
    defaultUsername: runtimeConfig.defaultUsername || 'demo',
    search: {
      provider: runtimeConfig.search?.provider || 'auto'
    },
    tools: {
      allowGhClone: runtimeConfig.tools?.allowGhClone === true
    },
    admin: {
      allowLoopbackBypass: runtimeConfig.admin?.allowLoopbackBypass === true
    }
  };
}
